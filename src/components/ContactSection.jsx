           ...
           <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {contactDetails.map((item) => (
                <FadeIn key={item.id} className="group">
                  <ContactCard icon={item.icon} title={item.title} content={item.content}/>
                </FadeIn>
              ))}
            </div>
...
const ContactCard = ({icon, title, content}) => {
  return( <div class="not-prose bg-background group/block-preview flex w-full max-w-full cursor-pointer select-none flex-col items-start justify-between hover:!rounded-[10px] rounded-3xl border transition-all duration-200 ease-in-out md:w-[400px] ring-alpha-400 border-gray-500 hover:!border-[#f4d7db] ring-2"><div class="bg-muted flex w-full items-center hover:!border-[#f4d7db] justify-between gap-2 !rounded-3xl p-2"><div class="size-4"><span class="text-gray-500">
    
    {icon && <div className='ml-1 w-[26px] w-[26px] rounded-full border border-gray-400 p-[6px]'>{icon}</div>}
    {/* <svg class="[&amp;_path]:fill-gray-500 size-4" data-testid="geist-icon" height="16" stroke-linejoin="round" viewBox="0 0 16 16" width="16"
 ><g clip-path="url(#clip0_872_3183)">
  ...
  <div class="my-0 flex w-full min-w-0 max-w-full items-center gap-2 text-sm font-medium text-gray-900">
 {  title &&  <p class="ml-1 text-xl font-display text-base/6 font-semibold tracking-wide text-black text-[#011211] min-w-0 shrink truncate">{title}</p>}
    <div class="bg-gray-150 flex h-5 items-center gap-1 rounded-md px-1 text-[12px] text-gray-600 font-mono">v1</div>
   </div>
    
    </div>
  <div class="w-full border-t border-gray-200"></div><div class="my-0 flex w-full max-w-full flex-col p-2 text-left text-gray-500"><button class="flex h-4 min-w-0 cursor-pointer items-center gap-2 font-mono text-xs hover:underline"><div class="flex size-4 shrink-0 items-center justify-center"><span class="relative flex size-[10px] items-center justify-center rounded-full bg-gray-300"><span class="bg-background absolute left-1/2 top-1/2 size-[8px] -translate-x-1/2 -translate-y-1/2 rounded-full"></span><span class="absolute left-1/2 top-1/2 z-10 size-[6px] -translate-x-1/2 -translate-y-1/2 rounded-full bg-gray-300"></span></span></div>
{ content &&  <div class="truncate text-left">{content}</div>}


  </button><div class="flex h-4 w-4 items-center justify-center"><hr class="h-full w-[1px] bg-gray-300"/></div><button class="flex h-4 min-w-0 cursor-pointer items-center gap-2 font-mono text-xs hover:underline"><div class="flex size-4 shrink-0 items-center justify-center"><span class="relative flex size-[10px] items-center justify-center rounded-full bg-gray-300"><span class="bg-background absolute left-1/2 top-1/2 size-[8px] -translate-x-1/2 -translate-y-1/2 rounded-full"></span><span class="absolute left-1/2 top-1/2 z-10 size-[6px] -translate-x-1/2 -translate-y-1/2 rounded-full bg-gray-300"></span></span></div><div class="truncate text-left">Generated page.tsx</div></button></div></div>)
}