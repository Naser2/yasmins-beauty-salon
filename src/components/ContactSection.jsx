...
const ContactCard = ({icon, title, content}) => {
  return( <div class="group not-prose bg-background group/block-preview 
    flex w-full max-w-full cursor-pointer select-none 
  flex-col items-start justify-between hover:!rounded-[10px] rounded-3xl 
  border transition-all duration-200 ease-in-out md:w-[400px]
   ring-alpha-400 border-gray-500 
  hover:!border-pink-400 ring-2 hover:!border-[#f4d7db] ">
    <div class="bg-muted flex w-full items-center hover:!border-pink-300 justify-between gap-2 !rounded-3xl p-2"><div class="size-4">
      <span class="text-gray-500">
    
    {icon && <div className='ml-1 w-[26px] w-[26px] rounded-full border border-gray-400 group-hover:border-[#f4d7db] p-[6px] group-hover:!text-pink-500'>{icon}</div>}
 ...
 {  title &&  <p class="ml-1 text-xl text-black/70 text-base/6 font-semibold tracking-wide text-black text-[#011211] group-hover:!text-pink-400  min-w-0 shrink truncate">{title}</p>}
  
   </div>
    
    </div>
  <div class="font-display  w-full border-t border-gray-200"></div><div class="my-0 flex w-full max-w-full flex-col p-2 text-left text-gray-500">
...
{ content &&  <div class="ml-4 truncate text-left text-[1.3em] text-black  font-bold font-[GOTHIC]">{content}</div>}
  ...
  </button><div class="flex h-4 w-4 items-center justify-center"></div><button class="flex h-4 min-w-0 cursor-pointer items-center gap-2 font-mono text-xs hover:underline"><div class="flex size-4 shrink-0 items-center justify-center"><span class="relative flex size-[10px] items-center justify-center rounded-full bg-gray-300"><span class="bg-background absolute left-1/2 top-1/2 size-[8px] -translate-x-1/2 -translate-y-1/2 rounded-full"></span><span class="absolute left-1/2 top-1/2 z-10 size-[6px] -translate-x-1/2 -translate-y-1/2 rounded-full bg-gray-300">
  </span></span></div><div class="truncate text-left"></div></button></div></div>)
}