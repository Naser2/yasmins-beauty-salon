...
export default function Intro() {
  ...
  const [logoPositionChange, setLogoPositionChange] = useState("mt-[30%] md:mt-[25%]");
  const [logoFont, setLogoFont] = useState("animated-logo"); // Initial animated font


  useEffect(() => {
    setTimeout(() => {
      setLogoColorChange(
        "transition duration-150 transform text-[wheat] text-5xl md:text-[3em] lg:text-[3.7em] xl:text-[5em] font-bold signature-font"
      );
    }, 2030);
  }, []);
  ...
  useEffect(() => {
    setTimeout(() => {
      setLogoPositionChange("md:top-[3%] lg:ml-[-16%] font-bold");
    }, 2700);
  }, []);
  useEffect(() => {
    // Change to default font after 3 seconds (3000ms)
    const timeout = setTimeout(() => {
      setLogoFont("signature-font"); // Replace with your app's default font class
    }, 2600);
      ...
      <FadeInStagger>
        <FadeIn>
          <div className={`${logoPositionChange} max-w-[90vw] section-header mx-auto mt-[36%] md:mt-[5%] lg:mt-[15%]`}>
            ...
            {/* <p className={clsx(tagColorChange, "mt-0 sm:mt-3 sm:mt-3  sm:hidden text-sm")}>
              NJ hair salon.
            </p> */}
        ...
        <div className="mx-auto max-w-7xl overflow-hidden px-6 pb-[4rem] pt-20 sm:pb-6 lg:px-8 absolute inset-x-0 sm:right-0 bottom-[-5vh] sm:bottom-[-2vh] md:bottom-[4vh] lg:bottom-[2vh] xl:bottom-[3vh] lg:!left-[-567px] xl:!left-[-667px]">
          <div className="mt-[2em] flex justify-center  gap-x-3 lg:gap-x-7">
            {footerNavigation.social.map((item) => (
              <a key={item.name} href={item.href} className="text-[wheat] hover:text-pink-600  bg-black/60 p-1  p-[0.5rem] rounded-full">
